...
setInterval(() => {
    http.get(`http://dkklll.glitch.me/`); /// حط اسم المشروع تبعك name تعديل مهم بدل
}, 280000);
...
client.on("message", async message => {
    ...
    if (message.content.startsWith(prefix + "bc")) {
        ...
        let args = message.content
            ...
            .slice(1)
            .join(" ");
        if (!message.author.id === "714906687533744248") return; ///تعديل مهم حط الايدي تبعك
...
client.on("ready", () => {
    client.user.setActivity(`𝐒𝐑 𝐒𝐄𝐑𝐕𝐄𝐑`, { ///تعديل مهم حالة البوت
        type: "Streaming"
    }); ///تعديل حالة البوت
});
